...
/**
 * Zobrist hash manager for the transposition tables.
 *
 */
public class ZobristHashCalculator {

	/**
	 * The random values for each position.
	 */
	...
	/**
	 * Create the random values with Random.nextLong().
	 */
	 ...
	 * @return return the Zobrist hash for the actual board.
	 */
	...
	public long updateHash(long hash, Move move, Piece piece) {
		...
		if (move != null && piece != null) {
			...
			if (takenOpponentsPiece != null) {
				hash ^= positionValue[takenOpponentsPiece.getRow()][takenOpponentsPiece.getCol()][getPieceValue(
						takenOpponentsPiece)];
			}
